...
playerSet = {
	...
	initialize : function(userid, prgid, test, sizeChange){
		...
		if(test) {
			this._playUrl = 'http://imgcdn.pandora.tv/gplayer/pandora_ChannelPlayer_Global.swf?' + 'userid=' + userid + '&prgid=' + prgid + '&sizeChange=' + sizeChange;
			this._test = "y";
		} else {
			if(userid == "lolkmp_pm") {
		...
		var browserLang = cookieSet.GetCookie('clientLang') || "jp";
	},
	...
	startPlay : function(){
		if(play_ing != false)
		{
			...
			try {
				jQuery("#endingDiv").css("display", "none");
				jQuery("#endingDiv").html("");
			} catch (e) { }
	...
	*/
			...
			if(extIcfSet == "midroll") {
				...
				if (cookieSet.GetCookie('extSet') == "1"){ 
					...
					jQuery("#watch-video").css("padding", "15px 0");
				}else{
			...
			success : function(res){
				...
				try {
					with( eventClass ) {
						...
						vSrc = res.vodSrc;
						vTime = res.runtime;
						vFlashUrl = 'http://imgcdn.pandora.tv/gplayer/pandora_ChannelPlayer_Global.swf?userid=' + ch_userid + '&prgid=' + prgid;
						imgSrc = res.Thumb;
						//_vOptF = true;
						vOptArr = {
								'v240':'', 
...
*/
	...
	endding : function(Mode, ch_userid, prgid){
		...
		var Obj = this;
		var endingStart = "";
		var strhtml = flashHtml("http://imgcdn.pandora.tv/gplayer/category/pandora_Endingstill.swf?other=&lang=jp&stillUrl=&stillTime=8&stillReCount=1&ending=" + Mode + "&ch_userid=" + ch_userid +"&prgid=" + prgid + "&sbs=false", 'enddingPlayer' );

		if (jQuery("#divPlayer").css("width") > "641") {
			jQuery(".admov").css({"width" : "800px", 'height':'480px', 'top':'15px', 'left':'90px', 'position':'absolute'});
		...
		try {
			//loadJs.loadJS("http://ads.pandora.tv/NetInsight/text/pandora_jp/jp_channel/main@endding_network");
			$("#endingDiv").html('<div id="endingDivLayer" style="position:absolute;top:0px;width:100%;"><div style="text-align:center;"><table style="width:100%;"><tr><td><IFRAME FRAMEBORDER=0 MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=NO WIDTH=300 HEIGHT=250 SRC="http://ads.pandora.tv/NetInsight/html/pandora_jp/player/player@ending_ifr"></IFRAME></td></tr></table></div></div>').show();
		} catch (e) {}		
