		...
		var eventClass = {
			vObj : "",
			eName : new Array("click", "play1", "pause", "ended", "suspend", "progress", "loadstart", "stalled", "loadedmetadata", "loadeddata", "playing", "canplay", "seeking", "seeked", "dureationchange", "abort", "waiting", "error", "nx_player", "gradient_bg"),
			videoTag : "",
			vType : "",
			vSrc : "",
			imgSrc : "",
			...
			vOptArr : {"v240":"", "v336":"", "v480":"", "v720":"", "vpay":""},
			_conType : "video",
			_fullCheckF : true,
				...
				try {
					switch ( e.target.className ){
						...
						case "big_play" : case "play" : 
							...
							if (eventClass.vOptArr.vpay && eventClass._currResol == "720P") {
								alert("프리미엄만 사용 가능함 ");
							} else {
								if (eventClass._sTime <= 0) {
...
//							break;
						case "nx_player" : case "gradient_bg" :
							...
							if (eventClass.playTimeObj == "") { 
								...
								if (eventClass._sTime <= 0) videoObj.load();
								eventClass.vPlay();								
							}
...
//				EventHandler.bind(window, 'orientationchange', function () {});
			},
			...
			//	console.log("event Type = " + e.type);
				...
				switch( evt.type ){
					...
					case "progress" :
						...
						break;
					case "loadedmetadata" : 
						if (eventClass._setAd == false &&  eventClass._sTime > 0) {
							eventClass.vObj.currentTime = parseInt(eventClass._sTime, 10);
						}
						break;
					case "loadeddata" :
						if (eventClass._setAd == false &&  eventClass._sTime > 0) {
					...
					case "play" :
						if(eventClass._setAd == false) {
							if (eventClass._sTime > 0) eventClass.vObj.currentTime = parseInt(eventClass._sTime, 10);
...
//					alert("msRequestFullscreen");
					...
					alert("The Browser Not Supported FullScreen Mode");
...
//					vPoster = "poster ='" + this.imgSrc + "'";
				...
				if( this._conType == "audio" ) {
					...
					if( navigator.userAgent.match(/iPhone/i) ) {	// IPhone 일 경우 Image 영역으로 교체
						...
						_playObjConType = "		<video id=\"" + objName + "\" autobuffer preload=\"none\" style=\"left:0; top:0; background-color:#000000;overflow: hidden; z-index: 2;width:" + this.vWidth + "; height:" + this.vHeight + ";\" " + vPoster + " src=\"" + this.vSrc + "\">";
						_playObjConType += "			<source src=\"" + this.vSrc + "\" type=\"" + this.vType + "\">";
//						_playObjConType += "			<track kind=\"subtitles\" label=\"English subtitles\" src=\"/m/js/vtt.ptv\" srclang=\"en\"></track>";
//						_playObjConType += "			<track kind=\"subtitles\" label=\"Korean subtitles\" src=\"/m/js/vtt_kr.ptv\" srclang=\"kr\" default></track>";
						_playObjConType += "	</video>";
					}
				}

				_playObj += "<div class=\"nx_player\" style=\"width:"+playerSize+"; height:100%; position:relative;\">";

				_playObj += "	<div class=\"gradient_bg\" id=\"gradient_bg\"></div>";
				_playObj += "	<div class=\"screen\" id=\"screen\">";
				_playObj += _playObjConType;
				_playObj += "	</div>";

				_playObj += "	<div id=\"adcontainer\"></div>";
				_playObj += "	<div id=\"companionDiv\"></div>";
			...
			//	_playObj += "			<strong class=\"loading_bubble\">16:47</strong>";				
				...
				if( this._conType == "video" )	_playObj += "<button class=\"full_screen\" id=\"video-fullscreen\" style=\"display:none\"></button>";
				...
				_playObj += "		<span id=\"vodeTime\">" + eventClass.timeFunc(eventClass.videoGetTimeFunc('T')) + "</span>";
				_playObj += "		<div class=\"pic_q\">";
				_playObj += "			<p class=\"clsBitTxt\">" + eventClass._currResol;
/*
...
//				_playObj += "			<ul class=\"clsBitrateSel\" style=\"display:" + ( this._vOptF ? "block" : "none" ) + "\">";
				...
				if (eventClass.vOptArr.vpay == undefined || eventClass.vOptArr.vpay == "") {
					_playObj += this.vOptArr.v720 ? "<li id=\"vO720\"" + (eventClass._currResol == "720P" ? " class=\"on\"" : "") + "><em></em>720P</li>" : "" ;
				} else {
					_playObj += this.vOptArr.vpay ? "<li><em></em><strike>720P</strike></li>" : "" ;
				}

				_playObj += this.vOptArr.v480 ? "<li id=\"vO480\"" + (eventClass._currResol == "480P" ? " class=\"on\"" : "") + "><em></em>480P</li>" : "" ;
				_playObj += this.vOptArr.v336 ? "<li id=\"vO336\"" + (eventClass._currResol == "336P" ? " class=\"on\"" : "") + "><em></em>336P</li>" : "" ;
				_playObj += this.vOptArr.v240 ? "<li id=\"vO240\"" + (eventClass._currResol == "240P" ? " class=\"on\"" : "") + "><em></em>240P</li>" : "" ;
			...
			videoGetTimeFunc : function( strV ) {
				...
				var totalTime = eventClass.vTime > 0 ?  Math.floor((eventClass.vTime / 1000 ) % 60000) : Math.floor(eventClass.vObj.duration);
...
//						self.setEvent();
			...
			adMezzo : function () {
				try {
					...
					mezzo.videoWrapperSelector =".nx_player";
