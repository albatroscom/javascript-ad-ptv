...
//			console.log("ad time : " + _adStime + " trueView time : " + _adJasonData["trueView"]["time"] );
	...
	adptv.onTimeUpdate = function (e) {
		if( parseInt(this.currentTime*10, 10)/10 >= parseInt(this.duration*10, 10)/10 ) {
			// 재생시간이 변경될 때 마다 현재시간, 전체길이를 0.1초 단위까지만 같은지 비교한 후 로직 실행.
				adptv.onEnded();
		}
	};
...
//								overlayWindow.open(tel, '_blank');
				...
				jQuery.each(result.icons, function(index, icon) {
					...
					// 아이콘 배치, 기존 x_ratio는 화면 비에 대한 비율임.(현재 사용안함.)
					switch(index) {
						case 0:
							$icon.css({
								left: "5px"
...
!function (name, context, definition) {
	...
	mezzo.run = function(data){
		...
		var mezzoTimeUpdate = function(e){
			if( parseInt(this.currentTime*10, 10)/10 >= parseInt(this.duration*10, 10)/10 ) {
				// 재생시간이 변경될 때 마다 현재시간, 전체길이를 0.1초 단위까지만 같은지 비교한 후 로직 실행.
					mezzoADEnd();
			}
